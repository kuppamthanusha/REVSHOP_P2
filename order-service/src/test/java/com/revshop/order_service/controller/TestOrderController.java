...
public class TestOrderController {
	...
	@Test
	public void testOrderHistory_UserIdNotFound() {
		Integer userId = null;
		
		ResponseEntity<?> res = orderController.orderHistory(userId);
		assertEquals(HttpStatus.BAD_REQUEST, res.getStatusCode());
	}
	
	@Test
	public void testOrderHistory_UserIdFound_UserNotFound() {
		Integer userId = 1;
		
		when(orderService.getUserById(userId)).thenReturn(null);
		ResponseEntity<?> res = orderController.orderHistory(userId);
		assertEquals(HttpStatus.NOT_FOUND, res.getStatusCode());
	}
	
	@Test
	public void testOrderHistory_OrderNotFound() {
		Integer userId = 1;
		
		when(orderService.getUserById(userId)).thenReturn(userModel);
		when(orderService.getOrdersByUser(userModel)).thenReturn(null);
		ResponseEntity<?> res = orderController.orderHistory(userId);
		assertEquals(HttpStatus.NOT_FOUND, res.getStatusCode());
	}
	
	@Test
	public void testOrderHistory_OrderFound() {
		Integer userId = 1;
		when(orderService.getUserById(userId)).thenReturn(userModel);
		...
		when(orderService.getOrdersByUser(userModel)).thenReturn(orders);
		ResponseEntity<?> res = orderController.orderHistory(userId);
		assertEquals(HttpStatus.OK, res.getStatusCode());
	}
}

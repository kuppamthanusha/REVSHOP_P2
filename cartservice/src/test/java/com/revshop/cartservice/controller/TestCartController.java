...
public class TestCartController {
	...
	@Test
	public void updateCart_ReturnsCartDelete() {
		int userId = 1;
		int productId = 1;
		int quantity = 0;
		Cart cart = new Cart();
	...
	@Test
	public void deleteCart_ReturnsOk() {
		Cart cart = new Cart();
		int userId = 1;
		int productId = 1;
		
		when(cartService.findByUserAndProduct(userId, productId)).thenReturn(cart);
		ResponseEntity<?> response = cartController.deleteFromCart(productId, userId);
		assertEquals(HttpStatus.OK, response.getStatusCode());
	}
	
	@Test
	public void deleteCart_ReturnsCartNotFound() {
